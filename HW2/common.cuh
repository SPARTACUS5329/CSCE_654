...
#define CUDA_CHECK(expr) do { \  cudaError_t _e = (expr); \  if (_e != cudaSuccess) { \    fprintf(stderr, "CUDA error %s:%d: %s\n", __FILE__, __LINE__, cudaGetErrorString(_e)); \    std::exit(1); \  } \} while(0)

#define CUBLAS_CHECK(expr) do { \  cublasStatus_t _s = (expr); \  if (_s != CUBLAS_STATUS_SUCCESS) { \    fprintf(stderr, "cuBLAS error %s:%d: status %d\n", __FILE__, __LINE__, (int)_s); \    std::exit(1); \  } \} while(0)

inline float elapsed_ms(cudaEvent_t a, cudaEvent_t b) {
  float ms=0.f; CUDA_CHECK(cudaEventElapsedTime(&ms,a,b)); return ms;
}

inline void cublas_rowmajor_dgemm(cublasHandle_t h, int m, int n, int k,
                                  const double* Arow, const double* Brow, double* Crow,
                                  double alpha=1.0, double beta=0.0)
{
  const double *Acol = Arow, *Bcol = Brow; double *Ccol = Crow;
