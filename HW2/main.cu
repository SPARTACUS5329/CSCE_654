...
int main(int argc, char **argv)
{
  ...
  {
    if (!strcmp(argv[i], "--task") && i + 1 < argc)
      task = argi(argv, i);
  ...
  fill_host(hA.data(), hA.size(), 1);
  fill_host(hB.data(), hB.size(), 2);
  fill_host(hQ.data(), hQ.size(), 3);
  fill_host(hK.data(), hK.size(), 4);
  fill_host(hV.data(), hV.size(), 5);
  ...
  // Currently cublas not working for rectangular matrices
  // I am testing it out
  ...
  { // blocked
    ...
    double mad = max_abs_diff_host(hC.data(), hCref.data(), hC.size());
    double gflops = (2.0 * (double)M * N * K) / (ms * 1e6);
    printf("[DGEMM tiled] tile = %d time=%.3f ms rate=%.2f GF/s diff=%.3e\n", TILE, ms, gflops, mad);
  }
  else if (task == 2)
  { // DGEMM cuBLAS
    CUDA_CHECK(cudaEventRecord(e0, stream));
  ...
  { // attention via DGEMM with your naive DGEMM or tiled DGEMM or cuBLAS
    int mode = (task == 3) ? 0 : (task == 4) ? 1
                                             : 2;
    CUDA_CHECK(cudaEventRecord(e0, stream));
    ...
    const char *tag = (mode == 0) ? "naive" : (mode == 1) ? "tiled"
                                                          : "cuBLAS";
